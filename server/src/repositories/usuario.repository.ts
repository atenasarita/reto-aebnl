...
export class OracleUsuarioRepository implements UsuarioRepository {
  ...
  async createUsuario(usuario: CreateUsuarioInput): Promise<Usuario> {
    ...
    try {
      ...
      const insertUsuarioResult = await connection.execute(
        insertUsuarioSql,
        {
      ...
      const idUsuario = Array.isArray(outBinds.id_usuario) ? outBinds.id_usuario[0] : outBinds.id_usuario;
